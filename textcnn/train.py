...
from textcnn.model import TextCNN, args
from textcnn.dataloader import MyDataset
from textcnn.config import RECORD_PATH
...
def load_model(model, epoch):
    return model if epoch == 0 else model.load_state_dict(
        torch.load(os.path.join(RECORD_PATH, get_model_name(model, epoch))))
